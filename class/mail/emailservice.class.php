<?php
    ...
    include_once("framework/class/mail/emailmessage.class.php" );
    ...
    class EmailService extends Object {
         ...
         * Constructor
         */
        function EmailService($serviceType = DEFAULT_EMAIL_SERVICE_TYPE, $smtpHost = false, $smtpPort = 25)
        {
            $this->Object();
        ...
        function sendMessage(&$message)
        {
            ...
            elseif ($this->_serviceType == "smtp")
            {
                ...
                if ($this->_smtpUseAuthentication)
                {
                    ...
                    {
                        throw(new Exception("EmailService::sendMessage: Please provide a username and a password if you wish to use SMTP authentication"));
                        $mail->SMTPAuth = false;
                    }
                    else
                    {
                ...
                {
                    throw(new Exception("EmailService::sendMessage: You should specify an SMTP server in order to send emails."));
                    return false;
                }
                else
                {
            ...
            {
                $mail->IsMail();
                throw(new Exception("EmailService::sendMessage: Unrecognized value of the email_service_type setting. Reverting to PHP built-in mail() functionality"));
            }

            if (!$mail->Send())
            {
                throw(new Exception("EmailService::sendMessage: Error sending message: " . $mail->ErrorInfo));
                die();
            }
        }
    }

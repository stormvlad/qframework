    ...
    class qGoogleSearchRequestParser extends qSearchRequestParser
    {
        ...
        */
        function parse($request)
        {
            ...
            $request = preg_replace_callback("/([^[:alnum:]_]+|^)?(\".+?\")([^[:alnum:]_]+|$)/si", array(&$this, "_replaceTextualTerms"), $request);

            $request = preg_replace_callback("/([^[:alnum:]_]+|^)?([\"+-]?.+?\"?)([^[:alnum:]_]+|$)/si", array(&$this, "_replaceTerms"), $request);

...
class sqliteStatement
{
	...
	function sqliteStatement( $sql, &$connection )
	{
	...
	function &do_query()
	{
		...
		$resultId = @sqlite_query( $this->sql, $this->connection );
		...
		$recordset = new sqliteResultSet( $resultId, $this->connection );
		...
		{
			case ADODB_FETCH_NUM: $recordset->fetchMode = SQLITE_NUM; break;
			case ADODB_FETCH_ASSOC:$recordset->fetchMode = SQLITE_ASSOC; break;
			default:
			case ADODB_FETCH_DEFAULT:
			case ADODB_FETCH_BOTH:$recordset->fetchMode = SQLITE_BOTH; break;
		}
...
class sqliteResultSet
{
	...
	function sqliteResultSet( $resultId, $connectionId )
	{
		$this->fields = array();
	...
	{
		return sqlite_rows_affected( $this->connectionId );
	} 
	...
	function close()
	{
		sqlite_free_result( $this->resultId );
		$this->fields = array();
		$this->resultId = false;
	} 
	...
	{
		$this->_numOfRows = sqlite_num_rows( $this->resultId );
	...
	function MoveNext()
	{
		if (@$this->fields =& sqlite_fetch_array($this->resultId,$this->fetchMode)) {
			$this->_currentRow += 1;
			return true;
		}
		if (!$this->EOF) {
	...
	function _seek($row)
	{
		...
		return @sqlite_seek($this->resultId,$row);
	}
	...
	function _fetch()
	{
		$this->fields = @sqlite_fetch_array($this->resultId,$this->fetchMode);
